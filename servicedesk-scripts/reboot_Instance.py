...
def main():
    ...
    instance = elb.describe_load_balancers(LoadBalancerNames=[elbname,])['LoadBalancerDescriptions'][0]['Instances'][0]['InstanceId']
    
    ec2 = session.client('ec2', region_name=region)
    ec2.reboot_instances(InstanceIds=[instance, ])

    print("Instance with Instance ID: {} is rebooted".format(instance))
    
    # Wait for the ELB health check to go OutOfService
   ...
   # wait for the health check to become InService
